...
test("Cursor and Copilot caches enforce observation count and wire payload bounds", async (t) => {
  ...
  assert.equal(
    cursorCapabilityUsage(cursorScan.sessions[0]!).length,
    MAX_CAPABILITY_USAGE_EVENTS,
  );
  ...
  assert.equal(
    copilotCapabilityUsage(copilotScan.sessions[0]!).length,
    MAX_CAPABILITY_USAGE_EVENTS,
  );

  const status = scanCapabilityUsage([
    cursorScan.sessions[0]!,
    copilotScan.sessions[0]!,
  ]);
