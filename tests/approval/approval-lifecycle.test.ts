...
import { FakeRelayClient, waitUntil, cfg } from "../support/approval/approval-lifecycle-fixtures";

test("scoped approval becomes durable before ACK and emits deterministic terminal", async () => {
  const { requestApproval } = await import("../../apps/bridge/src/approvals");
  const { approvalsStatus } = await import("../../apps/bridge/src/approvals/state");
...
test("open question sends only user.message reply lifecycle and terminal cleanup", async () => {
  const { askOpenQuestion } = await import("../../apps/mcp/src/http/create-server");
  const fake = new FakeRelayClient();
