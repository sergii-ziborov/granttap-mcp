...
import type { GrokBotEndpointBundle } from "../../apps/bridge/src/mesh/identity/endpoint";

function run(args: string[], config: string): Promise<{ code: number | null; stdout: string; stderr: string }> {
  const child = spawn(process.execPath, ["--import", "tsx", "apps/bridge/src/bin/cli/mesh-connect.ts", ...args], {
