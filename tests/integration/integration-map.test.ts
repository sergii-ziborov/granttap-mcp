...
import { parseIntegrationMap } from "../../apps/bridge/src/mesh/observed/integration-map";
...
import { clearIntegrationMapCache, readIntegrationMap } from "../../apps/bridge/src/mesh/observed/integration-map";
import { otherSide, otherSides, repositoryNames } from "../../apps/bridge/src/mesh/handoff/other-side";
...
test("an agent's scoped view names the claims next to its own by file and by module", () => {
  ...
  const claims = [
    claim("task-api", "apps/bridge/src/mesh/store/index.ts"),
    claim("task-worker", "apps/bridge/src/mesh/store/index.ts"),
    claim("task-worker", "apps/bridge/src/mesh/catalog/index.ts"),
    claim("task-worker", "README.md"),
  ];
  assert.deepEqual(scopedNeighbours({ claims }, "task-api").map((item) => [item.claim.resource, item.kind]), [
    ["apps/bridge/src/mesh/store/index.ts", "file"],
    ["apps/bridge/src/mesh/catalog/index.ts", "module"],
  ]);
