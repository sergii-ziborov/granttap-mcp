...
} from "../../apps/bridge/src/sessions/support/activity-helpers";
import { childTitle } from "../../apps/bridge/src/sessions/support/child-threads";
...
test("a background-task notice the host wrote as a user turn is shown as status, not as the person's words", () => {
  ...
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "user", text: notice, createdAt: 1, ordinal: 0 });
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "user", text: "Вот это таск нотификейшин не парсится", createdAt: 2, ordinal: 1 });
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "message", text: notice, createdAt: 3, ordinal: 2 });
...
test("the host's slash commands, their output, and its image notes are notices or nothing, never the person's words", () => {
  ...
  const out: import("../../packages/protocol/schema").ActivityEntry[] = [];
  const seen = new Set<string>();
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "user", text: "<command-name>/model</command-name><command-args>opus</command-args>", createdAt: 1, ordinal: 0 });
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "user", text: "[Image: original 10x10, displayed at 5x5.]", createdAt: 2, ordinal: 1 });
...
test("Cursor timestamp and user_query marks are not the message", () => {
  ...
  const out: import("../../packages/protocol/schema").ActivityEntry[] = [];
  const seen = new Set<string>();
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "user", text: wrapped, createdAt: 1, ordinal: 0 });
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "message", text: "<timestamp>Saturday, Sep 19, 2026, 5:11 PM (UTC+3)</timestamp>\nThe graph is the towers.", createdAt: 2, ordinal: 1 });
  pushEntry({ out: out, seen: seen, sessionId: "s", kind: "user", text: unclosed, createdAt: 3, ordinal: 2 });
