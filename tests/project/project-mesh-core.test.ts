...
import { classifyHumanAttention } from "../../apps/bridge/src/mesh/tasks/attention";
...
import { createHandoffWorktree } from "../../apps/bridge/src/mesh/runtime/worktree";
...
test("handoff creates a separate branch and worktree before target execution", async () => {
  ...
  const created = createHandoffWorktree({
    repository: root, worktreeRoot: worktrees, taskId: "task-123", provider: "codex", baseSha: sha, now,
  });
  assert.ok(created);
  ...
  assert.equal(createHandoffWorktree({
    repository: root, worktreeRoot: worktrees, taskId: "task-123", provider: "codex", baseSha: sha, now,
  }), undefined);
  assert.equal(createHandoffWorktree({
    repository: "/missing/repo", worktreeRoot: worktrees, taskId: "broken", provider: "codex", baseSha: sha, now,
  }), undefined);
});
