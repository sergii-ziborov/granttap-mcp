...
import { claudeToRequest } from "../../apps/bridge/src/host/adapters";
import { isUnanswered, requestApproval } from "../../apps/bridge/src/approvals";
...
test("claude hook abstains when the phone never answers", async (t) => {
  ...
  const child = spawn(
    process.execPath,
    ["--import", "tsx", "apps/bridge/src/bin/hooks/claude-hook.ts"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
