...
import { applyStoreDelta, deltaIsEmpty, storeDelta } from "../apps/bridge/src/mesh/store-sync";
import {
...
test("a change made here is laid over what another process wrote meanwhile", () => {
  ...
  assert.deepEqual(delta.claims.removed, ["gone"]);
...
test("a checkpoint branch is named for its moment, under the Task's name", () => {
  ...
  assert.equal(checkpointBranchName("task/1", at), "granttap/checkpoint/task-1-20260906T101500");
