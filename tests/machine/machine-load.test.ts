...
import { agentLanes } from "../../apps/bridge/src/machine-load/mcp/refresh";
...
describe("provider scan cost", () => {
  it("records the latest measured cost for each provider", async () => {
    const { providerScanCost, timedProviderScan } = await import("../../apps/bridge/src/machine-load/mcp/scan-cost");
...
describe("machine load assembly", () => {
  it("keeps process, scan, and token measurements independent", async () => {
    const { buildMachineLoad } = await import("../../apps/bridge/src/machine-load");
  ...
  it("averages monitor CPU over a stable window", async () => {
    const { monitorLoadSampler } = await import("../../apps/bridge/src/machine-load");
    let micros = 0;
    let clock = 0;
    let rss = 100;
