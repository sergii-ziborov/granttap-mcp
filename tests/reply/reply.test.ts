...
test("a home-screen phone message creates a persisted Codex task", async (t) => {
  ...
  const { createCodexSession } = await import(`../../apps/bridge/src/reply/index.ts?test=${Date.now()}`);
...
test("a new task without a selected project uses an isolated GrantTap workspace", async (t) => {
  ...
  const { createCodexSession } = await import(`../../apps/bridge/src/reply/index.ts?general=${Date.now()}`);
...
test("a phone-selected access level is passed to the resumed Codex task", async (t) => {
  ...
  const { saveRuntimeConfig } = await import(`../../apps/bridge/src/config/index.ts?access=${Date.now()}`);
  saveRuntimeConfig({
    enabled: true,
    excludedSessions: [],
  ...
  const { deliverToSession } = await import(`../../apps/bridge/src/reply/index.ts?access=${Date.now()}`);
...
test("an encrypted phone image becomes a Codex image input", async (t) => {
  ...
  const { createCodexSession } = await import(`../../apps/bridge/src/reply/index.ts?image=${Date.now()}`);
...
test("a phone image is passed to a new Claude Code task as local context", async (t) => {
  ...
  const { createClaudeSession } = await import(`../../apps/bridge/src/reply/index.ts?claudeImage=${Date.now()}`);
