...
} from "../../apps/bridge/src/approvals/state";
import {
  resolveCursorMcpCapability,
  resolveCursorMcpServer,
} from "../../apps/bridge/src/providers/cursor-mcp-policy";
import { mcpConfigDigest } from "../../apps/bridge/src/capabilities";
...
test("Cursor MCP resolver abstains when a command identifies multiple servers", (t) => {
  ...
  assert.equal(capability.configHash,
    mcpConfigDigest({ url: "https://mcp.example.test/context" }));
...
test("Cursor afterShell correlates a child event to its root approval", (t) => {
  ...
  const child = spawnSync(
    process.execPath,
    ["--import", "tsx", "apps/bridge/src/bin/hooks/cursor-after-shell.ts"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
