...
const monitorSource = (): string =>
  [
    "apps/bridge/src/monitor/index.ts",
    "apps/bridge/src/monitor/support/runtime.ts",
    "apps/bridge/src/monitor/handlers/catalog.ts",
...
describe("transient snapshots", () => {
  it("lets a sender opt out of the durable mailbox", async () => {
    const { sealEnvelope } = await import("../../../packages/core/relay-client");
...
describe("machine liveness heartbeat", () => {
  ...
  it("publishes transiently to the phone without scanning sessions", async () => {
    const { HEARTBEAT_INTERVAL_MS, publishHeartbeat } = await import("../../../apps/bridge/src/monitor/support/heartbeat");
    ...
    const { publishHeldHeartbeat, HELD_HEARTBEAT_TTL_MS } = await import("../../../apps/bridge/src/monitor/support/heartbeat");
  ...
  it("holds one intro heartbeat so a phone that joins after pairing still sees the Mac", async () => {
    const { HELD_HEARTBEAT_TTL_MS, publishHeldHeartbeat } = await import("../../../apps/bridge/src/monitor/support/heartbeat");
    const sent: Array<{ payload: any; to: unknown; options: any }> = [];
    await publishHeldHeartbeat({
      send: async (payload: any, to: unknown, options: any) => {
        sent.push({ payload, to, options });
...
describe("scan pressure", () => {
  it("schedules the next scan only after the previous one finishes", async () => {
    const { startPublishLoop } = await import("../../../apps/bridge/src/monitor/support/publish-loop");
  ...
  it("keeps expensive scans single-flight and preserves a forced refresh", async () => {
    const { singleFlightPublisher } = await import("../../../apps/bridge/src/monitor/support/single-flight");
    const releases: Array<() => void> = [];
    const calls: boolean[] = [];
  ...
  it("re-reads a provider transcript only when that session moved", async () => {
    const { sessionActivityCache } = await import("../../../apps/bridge/src/monitor/support/session-activity");
    let reads = 0;
    const session = {
      sessionId: "chat-a",
