...
test("an attachment sent ahead of its message is kept until the message names it", async (t) => {
  ...
  const { ATTACHMENT_TTL_MS, pruneAttachments, storeAttachment, takeAttachment } =
    await import(`../../apps/bridge/src/delivery/attachment-store.ts?store=${Date.now()}`);
  const upload = {
...
`, { mode: 0o755 });
  ...
  const { resolveMessageAttachments, startSessionMonitor, sweepAttachments } = await import(`../../apps/bridge/src/monitor/index.ts?att=${Date.now()}`);
