...
import {
  ...
  guessRisk,
} from "../../apps/bridge/src/host/adapters";
...
test("chat capability controls and CLI deep links stay scoped inside E2EE", () => {
  ...
  const envelope = encryptedEnvelope({ payload: scoped, room: config.room, from: "machine", to: "phone", senderSecretKey: machine.secretKey, recipientPublicKey: phone.publicKey });
