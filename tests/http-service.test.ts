...
test("authorize restores a pre-existing owned plist when repaired service health fails", async (t) => {
  ...
  const env: NodeJS.ProcessEnv = {
    ...authorizeEnv(root, port),
    GRANTTAP_SKIP_LAUNCHCTL: "1",
  };
  const plistPath = join(env.GRANTTAP_LAUNCH_AGENTS_DIR!, "com.granttap.mcp-http.plist");
  ...
  assert.equal(existsSync(plistPath), true, "pre-existing service plist must not be deleted");
...
test("authorize restores exact owned plist and loaded state after bootstrap fails", async (t) => {
  ...
  writeFileSync(join(fakeBin, "launchctl"), [
    "#!/bin/sh",
    'case "$1" in',
    '  print) test -f "$GRANTTAP_FAKE_SERVICE_STATE" ;;',
    '  bootout) rm -f "$GRANTTAP_FAKE_SERVICE_STATE"; exit 0 ;;',
    "  bootstrap)",
    '    if ! test -f "$GRANTTAP_FAKE_FAILED_ONCE"; then',
    '      touch "$GRANTTAP_FAKE_FAILED_ONCE"',
    '      echo "injected bootstrap failure" >&2',
    "      exit 1",
    "    fi",
    '    touch "$GRANTTAP_FAKE_SERVICE_STATE"',
    "    exit 0",
    "    ;;",
    "esac",
    "",
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    GRANTTAP_FAKE_SERVICE_STATE: serviceState,
    GRANTTAP_FAKE_FAILED_ONCE: failedOnce,
  };
  const plistPath = join(env.GRANTTAP_LAUNCH_AGENTS_DIR!, "com.granttap.mcp-http.plist");
  ...
  assert.equal(readFileSync(plistPath, "utf8"), before);
  assert.equal(existsSync(serviceState), true, "previously loaded service must be reloaded");
});
