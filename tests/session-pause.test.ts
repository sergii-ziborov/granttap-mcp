...
test("a paused chat is refused every tool by every hook until it is resumed", async (t) => {
  ...
  } = await import(`../apps/bridge/src/config.ts?pause=${Date.now()}`);
...
test("a pause stops the delivery in flight and refuses a new one", async (t) => {
  ...
  const { abortProcesses, runProcess, runningProcessCount, STOPPED_ERROR } =
    await import(`../apps/bridge/src/reply/process.ts?pause=${Date.now()}`);
  ...
  const { setSessionPaused } = await import(`../apps/bridge/src/config.ts?pause2=${Date.now()}`);
  const { deliverToSession, stopDeliveries } = await import(`../apps/bridge/src/reply.ts?pause=${Date.now()}`);
...
`, { mode: 0o755 });
  ...
  const { handleSessionControl, RESUME_PROMPT } = await import(`../apps/bridge/src/monitor.ts?pause=${Date.now()}`);
  const { loadRuntimeConfig } = await import(`../apps/bridge/src/config.ts?pause3=${Date.now()}`);
