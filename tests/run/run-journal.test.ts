...
} from "../../apps/bridge/src/mesh/admin/journal";
import { meshBrief, meshMap } from "../../apps/bridge/src/mesh/snapshot/map";
import { promptContext } from "../../apps/bridge/src/mesh/context/prompt";
import { digestRun, noteDeliveredRun } from "../../apps/bridge/src/mesh/delivery/run-digest";
...
test("a finished delivery is digested from the transcript window and carried as Task progress", async (t) => {
  ...
  const cut = noteDeliveredRun({
    ...
    result: { ok: false, error: "claude did not respond within 600s." },
    startedAt: at,
    endedAt: at + 600_000,
    deps: {
...
test("the prompt hook binary adds the journal once, stays quiet inside a delivery, and never fails a prompt", async (t) => {
  ...
  const hook = (stdin: string, extraEnv: Record<string, string> = {}) => spawnSync(
    process.execPath, ["--import", "tsx", "apps/bridge/src/bin/hooks/claude-prompt-hook.ts"],
