...
Call the GrantTap MCP `connect` tool immediately. It reuses the existing secure
machine pairing. When no pairing exists, it returns a one-time QR image and a
manual fallback for the user to open in GrantTap on iPhone.
...
`npx -y granttap-mcp@0.8.6 setup` locally. Provider authentication remains
