...
import { relay, resetRelay } from "../../http/create-server";
import { buildConnectSnapshot } from "../session/connect-snapshot";
import { watchMailboxClaim } from "../session/mailbox-claim";
...
import { isAllowedLoopbackOrigin, isWebsiteOrigin, WEBSITE_ORIGINS } from "../session/loopback-origin";
...
export function installPairingRoutes(app: Express, provider: GrantTapOAuthProvider, issuerOrigin: string): void {
  app.post("/oauth/pairing", async (req, res) => {
    ...
    try {
      ...
      resetRelay();
      void relay();
      reloadPairingHelper({ firstPairing, replace });
