...
test("pending pairing refreshes while visible and host notifications update the card", async (t) => {
  ...
  assert.equal(ui.el("status").textContent, "Waiting for iPhone");
  notify("ui/notifications/tool-result", { structuredContent: { status: "connected", phoneLastSeenAt: Date.now() } });
  assert.equal(ui.el("status").textContent, "iPhone activity confirmed");
