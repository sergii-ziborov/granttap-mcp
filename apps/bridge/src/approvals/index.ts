...
export async function requestApproval(
  ...
  try {
    ...
    const waiting = waitForDecision(client, req, timeoutMs, registrationHandle);
    cancelDecisionWait = waiting.cancel;
    await sendRequest(client, req, timeoutMs);
...
async function sendRequest(client: RelayClient, req: ApprovalRequest, timeoutMs: number): Promise<void> {
  // The id carries no task content and lets APNs answer before iOS decrypts the card.
  const options = { ttlMs: timeoutMs, wake: true, deliveryId: req.requestId };
  if (req.sessionId) await sendSessionPayload(client, req, req.sessionId, "phone", options);
  else await client.send(req, "phone", options);
}

function waitForDecision(
  client: RelayClient, req: ApprovalRequest, timeoutMs: number,
  registrationHandle?: ApprovalRegistrationHandle,
): { promise: Promise<ApprovalDecision | null>; cancel: () => void } {
  let cancel = () => {};
