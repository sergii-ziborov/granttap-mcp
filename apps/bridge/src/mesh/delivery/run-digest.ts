...
import { stripAttachmentNote } from "../../sessions/support/activity-helpers";
import type { ReplyResult } from "../../reply/payload/types";
import { liveExecutionScope } from "../catalog/capability";
import { describeRun, recordRun, type RunRecord } from "../admin/journal";
 ...
 */
export function noteDeliveredRun(input: {
  session: SessionInfo;
  prompt: string;
  result: ReplyResult;
  startedAt: number;
  ...
  const { session, prompt, result, startedAt, endedAt, deps = liveDeps } = input;
  try {
