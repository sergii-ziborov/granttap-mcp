...
import { claudeTranscriptPaths } from "../../sessions/scan/claude";
import { codexTranscriptPaths } from "../../sessions/scan/codex";
...
import { readInvocationBatch, type InvocationLogCursor } from "./log-reader";
import {
  parseInvocationLine, sourceGapFact, type InvocationFact, type InvocationParseState,
} from "./parse";
import type { InvocationEvent } from "../protocol/engine-invocation-protocol";
import { parseInvocationDecision } from "./decision-parse";
          ...
          for (const record of records) {
            ...
            for (const fact of observed) {
              events.push(eventFor({
                session, provider, computer: options.computer(), thread,
                offset: record.offset, fact, repository: facts,
              }));
            }
          }
          try {
...
function eventFor(input: {
  ...
  const { session, provider, computer, thread, offset, fact, repository } = input;
