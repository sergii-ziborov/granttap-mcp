...
function tryAcquireLock(input: {
  ...
  const { path, token, primary, read, write, expired, ownerIsPrimary, reportRefusal } = input;
  if (input.getLeader() && read() === token) {
    write();
    return true;
  }
...
export function monitorLeadership(
  ...
  const acquire = (): boolean => tryAcquireLock({
    path, token, primary, read, write, expired, ownerIsPrimary, reportRefusal,
    getLeader: () => leader,
    setLeader: (value) => { leader = value; },
  });
